  ...
  const getData = async () => {
    try {
      const response = await UseAxios({
        method: "GET",
        api: `getWorkShopStoreProduct/${workshop}?${filter?.search ? `?product_name=${filter.search}&` : ""}
page=${currentPage}&
    ...
    <div style={{display:"flex",justifyContent:"space-around"}}>
     ...
     <p>{item?.product_price||item?.product?.price }$</p>
              ...
              <div style={{display:"flex",justifyContent:"center"}}>
               <ProductUnitDialog workshop={workshop} product_id={item?.product?.id}/>
              </div>
    </div>
