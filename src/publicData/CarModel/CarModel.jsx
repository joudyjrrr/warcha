...
function CarModel() {
 ...
 await UseAxios({method:"delete",api:`DeleteCarModel/${id}`})
    setOpen(true);
    setTimeout(() => {
    location.reload();
  }, 3000);
 ...
 const handleUpdate = (e) => {
        e.preventDefault();
   UseAxios({ method: "post", api: `updateCarModel/${data[index].id}`, data: { value: data[index].value } })
  setOpen(true)   
  setTimeout(() => {
      location.reload();
     }, 2000);     
...
try {
  const res =await UseAxios({ method: "get", api: "getCarModel" })
