...
function Shift() {
 ...
 const handleUpdate = (e) => {
        e.preventDefault();
   UseAxios({ method: "post", api: `updateShift/${data[index].id}`, data: { name: data[index].name,start_time:data[index].start_time,end_time:data[index].end_time } })
  setOpen(true)   
  setTimeout(() => {
      location.reload();
     }, 2000);     
...
try {
  const res =await UseAxios({ method: "get", api: "getShift" })
  ...
  <tbody >
     ...
     <FormControl onChange={(e)=>{data[index].name=e.target.value}} style={{textAlign:"center"}} defaultValue={item?.name} type="text" required/>
       :item?.name
                 ...
                 <th>
               {index == indexof ? 
     <FormControl onChange={(e)=>{data[index].start_time=e.target.value}} style={{textAlign:"center"}} defaultValue={item?.start_time} type="time" required/>
       :item?.start_time
 }
                </th>
                  <th>
               {index == indexof ?
     <FormControl onChange={(e)=>{data[index].end_time=e.target.value}} style={{textAlign:"center"}} defaultValue={item?.end_time} type="time" required/>
       :item?.end_time
 }
               </th>
      <th>
