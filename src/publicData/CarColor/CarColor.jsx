...
import CreateCarColor from "./CreateCarColor"
...
function CarColor() {
 ...
 await UseAxios({method:"delete",api:`DeleteCarColor/${id}`})
    setOpen(true);
    setTimeout(() => {
    location.reload();
  }, 3000);
 ...
 const handleUpdate = (e) => {
        e.preventDefault();
   UseAxios({ method: "post", api: `updateCarColor/${data[index].id}`, data: { color: data[index].color,code:data[index].code } })
  setOpen(true)   
  setTimeout(() => {
      location.reload();
     }, 2000);     
...
try {
  const res =await UseAxios({ method: "get", api: "getCarColor" })
        ...
        {data.map((item, indexof) => {
            return <>
             <tr key={indexof}>
              <th>
               {index == indexof ?
     <FormControl onChange={(e)=>{data[index].color=e.target.value}} style={{textAlign:"center"}} defaultValue={item?.color} type="text" required/>
       :item?.color
 }
                </th>
                      <th style={{display:"flex",justifyContent:"center"}}>
               {index == indexof ?
                    <FormControl onChange={(e) => { data[index].code = e.target.value }}
                      style={{ textAlign: "center" }} defaultValue={item?.code} type="color" required />
       : <FormControl
                      style={{ textAlign: "center" }} defaultValue={item?.code} type="color" disabled required />
 }
               </th>
      <th>
