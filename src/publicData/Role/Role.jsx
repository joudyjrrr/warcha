...
const AddProductCategory = () => {
 ...
 const handleSubmit = (e) => {
     e.preventDefault();
   UseAxios({ method: "post", api: "createRole", data: { name: name } })
   setOpen(true)
     setTimeout(() => {
      location.reload();
     }, 2000);    
 ...
 <Form onSubmit={(e)=>{handleSubmit(e)}}>
  <h3 style={{textAlign:"center"}}>Add New</h3>
    <FormGroup style={{width:"80%",margin:"0 auto"}}>
     ...
     <FormControl onChange={(e)=>{setName(e.target.value)}}  type="text" required/>
   </FormGroup>
...
function Role() {
 ...
 const handleUpdate = (e) => {
        e.preventDefault();
   UseAxios({ method: "post", api: `updateRole/${data[index].id}`, data: { name: data[index].name } })
  setOpen(true)   
  setTimeout(() => {
      location.reload();
     }, 2000);     
...
try {
  const res =await UseAxios({ method: "get", api: "getRole" })
