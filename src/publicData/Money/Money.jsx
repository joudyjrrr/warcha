...
function Money() {
 ...
 const handleUpdate = (e) => {
   e.preventDefault();
   console.log();
   UseAxios({ method: "post", api: `updatePublicData/${data[index].id}`, data: { dollar_price: data[index].dollar_price,currency:data[index].currency } })
  setOpen(true)   
  setTimeout(() => {
      location.reload();
     }, 2000);     
        ...
        {data.map((item, indexof) => {
            return <>
             <tr key={indexof}>
              <th>
               {index == indexof ?
     <FormControl onChange={(e)=>{data[index].currency=e.target.value}} readOnly={item.currency=="$"} style={{textAlign:"center"}} defaultValue={item?.currency} type="text" required/>
       :item?.currency
 }
                </th>
                 <th>
               {index == indexof ?
     <FormControl onChange={(e)=>{data[index].dollar_price=e.target.value}} readOnly={item.currency=="$"} style={{textAlign:"center"}} defaultValue={item?.dollar_price} type="number" required/>
       :item?.dollar_price
 }
               </th>
      <th>
