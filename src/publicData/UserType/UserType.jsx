...
import CreateEmployeeType from "./UserTypeDialog"
function UserType() {
 ...
 const handleUpdate = (e) => {
        e.preventDefault();
   UseAxios({ method: "post", api: `updateEmployeeType/${data[index].id}`, data: { name: data[index].name } })
  setOpen(true)   
  setTimeout(() => {
      location.reload();
     }, 2000);     
...
try {
  const res =await UseAxios({ method: "get", api: "getEmployeeType" })
