  ...
  const getData = async () => {
    try {
      ...
      const response = await UseAxios({
        method: "GET",
        api: `getBranch?${filter?.name ? `?name=${filter.name}` : ""}
${pagenate ? `&paginate=${pagenate}` : ""}
${page ? `&page=${page}` : ""}
${filter?.admin_id ? `&admin_id=${filter.admin_id}` : ""}`
      })
   ...
   <div className="tableContent">
    ...
    <MenuItem color="warning" >ALL</MenuItem>
                ...
                {admin.map((item,index) => {
                  return <MenuItem color="warning"  value={item?.get_admin?.branch_admin?.user_id} key={index}>{item?.get_admin?.branch_admin?.first_name } {item?.get_admin?.branch_admin?.last_name }</MenuItem>
               })}
        ...
        {data?.map((item, index) => {
            return <>
              <tr key={index}>
                ...
                <th>{item?.name }</th>
                <th>{item?.get_admin?.branch_admin?.first_name } {item?.get_admin?.branch_admin?.last_name }</th>
                <th>{item?.branch_balance }</th>
                <th>{item?.creditor }</th>
                <th>{item?.debtor }</th>
                <th>
                  <Switch
...
<svg onClick={() => { nav("/Branch", { state: { branch: item.id } }); }} width="40" height="40" viewBox="0 0 21 20" fill="none" xmlns="http://www.w3.org/2000/svg">
