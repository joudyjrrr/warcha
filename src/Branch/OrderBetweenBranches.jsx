...
import ReserverOrder from "./ReserverOrder";
import SendingOrder from "./SendingOrder";
const OrderBetweenBranches = () => {
 ...
 const [filter, setFillter] = useState();
  ...
  const [currentPage, setPagePage] = useState(1);
 const [order, setOrder] = useState([]);
 const [information, setInformation] = useState([]);
 const [totalPrice, setTotalPrice] = useState(0);
 const [message, setMessage] = useState("success");
  const [open, setOpen] = useState(false);
  const [branchChose, setBranch] = useState();
  const [branches, setBranches] = useState([]);
 const handelAdd = async () => {
 try {
   await UseAxios({
   method: "post", api: "orderFromOtherBranch",
     data: {
       "order": order,
       "sender_id": branchChose,
       "reverse_id": information?.reverse_id
     }
  });
  setMessage("success")
  setOpen(true)
...
${filter?.category_id ? `product_category_id=${filter.category_id}` : ""}`
...
}, [pagenate,filter?.category_id, enter,currentPage,branchChose,total,enter]);
  return (
    
   <>
      <div className="filterContent">
                     <ul className="selectElement" style={{justifyContent:"center"}}>
     <li onClick={()=>setPage(0)} className={page==0?"underLine":null} style={{marginRight:"20px"}}>NEW ORDER</li>
     <li onClick={()=>setPage(1)} className={page==1?"underLine":null} style={{marginRight:"20px"}}>SENDING INFORMATION</li>
     <li onClick={()=>setPage(2)} className={page==2?"underLine":null} style={{marginRight:"20px"}}>Reception INFORMATION</li>
     {/* <li onClick={() => setPage(3)} className={page == 3 ? "underLine" : null} style={{ marginRight: "20px" }}>BRANCH MONEY</li> */}
 ...
 <>
  ...
  <Autocomplete
    ...
    onChange={(e, selectedOption) => {
      if (selectedOption) {
        setInformation({...information,reverse_id:selectedOption.id});
      } else {
        setBranch(null); // handle case when no option is selected
      }
    }}
 ...
 const handleDecrement = () => {
  ...
  return (
    ...
    <h6 >total ={totalPrice }$</h6>
     ...
     <button disabled={currentPage<=1? true : false} onClick={() => { setPagePage(currentPage - 1)}}><svg width="16" height="16" viewBox="0 0 16 16" fill="none" xmlns="http://www.w3.org/2000/svg">
<path d="M10 4L6 8L10 12" stroke="#969AA0" strokeWidth="1.75" strokeLinecap="round" strokeLinejoin="round"/>
</svg>
</button>
       {currentPage != 1?<button onClick={() => { setPagePage(1) }} className={currentPage == 1 ? "pagenateFlex" : null}>{1 }</button> :null}
       <button className="pagenateFlex">{currentPage}</button>
       {currentPage !=total&&<button onClick={() => { setPagePage(total) }}>{total }</button>}
      <button 
  disabled={currentPage >= total?true:false}
  onClick={() => {
   setPagePage(currentPage + 1);
  }}
...
export default OrderBetweenBranches